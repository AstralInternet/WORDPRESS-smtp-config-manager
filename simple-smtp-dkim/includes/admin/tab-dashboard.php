...
<div class="simple-smtp-dkim-card">
    ...
    <div class="smtp-progress-bar-wrap" role="progressbar" aria-valuenow="<?php echo esc_attr($pct); ?>" aria-valuemin="0" aria-valuemax="100" aria-label="<?php esc_attr_e('Setup progress', 'simple-smtp-dkim'); ?>">
        <div class="smtp-progress-bar" style="width:<?php echo esc_attr($pct); ?>%">
            <span class="smtp-progress-text"><?php echo esc_html(sprintf('%d%%', $pct)); ?></span>
        </div>
    </div>
    <?php /* translators: %1$d: completed steps, %2$d: total steps */ ?>
    <p class="smtp-progress-summary"><?php echo esc_html(sprintf(__('%1$d of %2$d steps completed', 'simple-smtp-dkim'), $done_count, $total)); ?></p>
...
<!-- Quick Stats -->
<div class="simple-smtp-dkim-card">
    <h2><?php esc_html_e('Email Activity — Last 30 Days', 'simple-smtp-dkim'); ?></h2>
    <div class="smtp-stats-grid">
        <div class="smtp-stat-box"><div class="smtp-stat-number"><?php echo esc_html(number_format($stats['total'])); ?></div><div class="smtp-stat-label"><?php esc_html_e('Total', 'simple-smtp-dkim'); ?></div></div>
        <div class="smtp-stat-box smtp-stat-success"><div class="smtp-stat-number"><?php echo esc_html(number_format($stats['success'])); ?></div><div class="smtp-stat-label"><?php esc_html_e('Sent', 'simple-smtp-dkim'); ?></div></div>
        <div class="smtp-stat-box smtp-stat-error"><div class="smtp-stat-number"><?php echo esc_html(number_format($stats['failed'])); ?></div><div class="smtp-stat-label"><?php esc_html_e('Failed', 'simple-smtp-dkim'); ?></div></div>
        ...
        <div class="smtp-stat-box"><div class="smtp-stat-number"><?php echo esc_html(number_format($stats['dkim_signed'])); ?></div><div class="smtp-stat-label"><?php esc_html_e('DKIM', 'simple-smtp-dkim'); ?></div></div>
    </div>

    <?php $recent_errors = Simple_SMTP_DKIM_Logger::get_recent_errors(5);
    if (!empty($recent_errors)): ?>
    <h3><?php esc_html_e('Recent Failures', 'simple-smtp-dkim'); ?></h3>
    <table class="widefat fixed striped">
        <thead><tr><th><?php esc_html_e('Date', 'simple-smtp-dkim'); ?></th><th><?php esc_html_e('To', 'simple-smtp-dkim'); ?></th><th><?php esc_html_e('Error', 'simple-smtp-dkim'); ?></th></tr></thead>
        <tbody>
    ...
    <p><a href="<?php echo esc_url(admin_url('options-general.php?page=simple-smtp-dkim&tab=logs')); ?>"><?php esc_html_e('View all logs →', 'simple-smtp-dkim'); ?></a></p>
    <?php endif; ?>
</div>
<?php else: ?>
<div class="simple-smtp-dkim-card">
    <p class="smtp-logging-disabled">
        ...
        <a href="<?php echo esc_url(admin_url('options-general.php?page=simple-smtp-dkim&tab=logs')); ?>"><?php esc_html_e('Enable it to see statistics.', 'simple-smtp-dkim'); ?></a>
    </p>
</div>
<?php endif; ?>
...
<div class="simple-smtp-dkim-card">
    ...
    <table class="smtp-summary-table">
        <tr><th><?php esc_html_e('SMTP', 'simple-smtp-dkim'); ?></th><td><?php Simple_SMTP_DKIM_Helpers::render_badge(get_option('simple_smtp_dkim_enabled')); ?></td></tr>
        <tr><th><?php esc_html_e('Host', 'simple-smtp-dkim'); ?></th><td><?php echo esc_html(get_option('simple_smtp_dkim_host', '—')); ?></td></tr>
        <tr><th><?php esc_html_e('Port / Encryption', 'simple-smtp-dkim'); ?></th><td><?php echo esc_html(get_option('simple_smtp_dkim_port', '—') . ' / ' . strtoupper(get_option('simple_smtp_dkim_secure', 'none'))); ?></td></tr>
        <tr><th><?php esc_html_e('From', 'simple-smtp-dkim'); ?></th><td><?php echo esc_html(get_option('simple_smtp_dkim_from_email', '—')); ?></td></tr>
        <tr><th><?php esc_html_e('DKIM', 'simple-smtp-dkim'); ?></th><td><?php Simple_SMTP_DKIM_Helpers::render_badge(get_option('simple_smtp_dkim_dkim_enabled')); ?></td></tr>
        <?php if (get_option('simple_smtp_dkim_dkim_enabled')): ?>
        <tr><th><?php esc_html_e('DKIM Domain', 'simple-smtp-dkim'); ?></th><td><?php echo esc_html(get_option('simple_smtp_dkim_dkim_domain', '—')); ?></td></tr>
        <?php endif; ?>
    </table>
</div>
