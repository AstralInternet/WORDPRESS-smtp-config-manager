...
= 1.1.0 =
* Complete admin interface redesign: modern card-based layout, pill navigation, status banners and badges
* New guided setup wizard (provider presets, credentials, live test) launched from the dashboard
* New animated 7-step delivery diagnostic (DNS, connection, TLS, authentication, SPF, DKIM, real test send)
* Email Logs: filter pills with live counts, redesigned list, detail slide-over panel, server-side CSV export of all matching logs
* Dashboard: progress ring, setup checklist, activity stats, recent failures
* Self-hosted Figtree and JetBrains Mono fonts (no external requests, GDPR-friendly)
* Fixed: scheduled daily log purge was never running
* Fixed: uninstall cleanup removed the wrong table name and wrong upload directory
* Fixed: encrypted secrets were stored with autoload enabled (memory waste on every page)
* Fixed: the "enabled" flag is now autoloaded (one less DB query on every page load)
* Fixed: OAuth field-visibility script crashed (jQuery scope error)
* Fixed: "Send test email" ignored a disabled authentication setting
* Accessibility: focus traps in dialogs, aria-live regions, reduced-motion support
...
= 1.1.0 =
Major interface redesign with guided setup wizard and step-by-step delivery diagnostic, plus several bug fixes (log purge cron, uninstall cleanup, option autoload).
